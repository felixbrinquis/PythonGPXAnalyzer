...
from source.common.PaletasColores import PinkRose

def Pendiente(DatosBokeh, EjeX, MetricasAuxiliares):
    """
        GRAFICO AREA | PENDIENTE
    """
    ...
    PLT = figure(width=1000, height=400, x_range=(DatosBokeh.data[EjeX].min(), DatosBokeh.data[EjeX].max()), y_range=(LimiteEjeY(DatosBokeh, 'PENDIENTE', 'Inferior'), LimiteEjeY(DatosBokeh, 'PENDIENTE', 'Superior')), tools='', toolbar_location=None)
    
	# Creacion del area bajo la linea de la metrica a partir del CDS
    Area = DatosBokeh.to_df().copy().reset_index()[[EjeX, 'Pendiente[%]']].set_index(EjeX)
    Area.rename(columns= {'Pendiente[%]': 'Area'}, inplace=True)
    AreaBottom = Area[::-1]
    AreaBottom['Area'] = 0
    PLT.patch(x= hstack((AreaBottom.index, Area.index)), y= hstack((AreaBottom['Area'], Area['Area'])), color= PinkRose[0], alpha=1, line_color=None)
	
    # Inclusion de datos
    PLT_Linea = PLT.line(EjeX, 'Pendiente[%]', source=DatosBokeh, color=PinkRose[1], line_width=2, line_cap='round')
    PLT.add_tools(HoverTool(tooltips=[('', '@{Pendiente[%]}{0.00}%')], renderers=[PLT_Linea], mode='vline'))
    ...
    for Metrica in MetricasAuxiliares:
        if DiccionarioVariables[Metrica]['Tipo'] == 'circle':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.circle(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['PENDIENTE']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], size=SizeCircle, line_color=DiccionarioVariables[Metrica]['Color'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        elif DiccionarioVariables[Metrica]['Tipo'] == 'step':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.step(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['PENDIENTE']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        else:
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.line(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['PENDIENTE']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
    
    # Atributos
    PLT.title.text = 'PENDIENTE'
    PLT.sizing_mode = 'fixed'
    PLT.yaxis.axis_label = 'Pendiente [%]'
    PLT.yaxis.formatter = NumeralTickFormatter(format= '0,0')
    PLT.grid.visible = False
    PLT.yaxis.minor_tick_line_color = None
    PLT.yaxis.major_label_overrides = FormateoEjes(DatosBokeh.data['Pendiente[%]'], 1, 1)
