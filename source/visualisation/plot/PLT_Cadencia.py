...
def Cadencia(DatosBokeh, EjeX, MetricasAuxiliares):
    """
        GRAFICO DISCRETO | CADENCIA
    """
    ...
    PLT = figure(width=1000, height=400, x_range=(DatosBokeh.data[EjeX].min(), DatosBokeh.data[EjeX].max()), y_range=(LimiteEjeY(DatosBokeh, 'CADENCIA', 'Inferior'), LimiteEjeY(DatosBokeh, 'CADENCIA', 'Superior')), tools='', toolbar_location=None)
    
    # Inclusion de datos
    PLT.circle(EjeX, 'Cadencia[ppm]', source=DatosBokeh, size=SizeCircle, line_color=transform('Cadencia[ppm]', LinearColorMapper(palette=PaletaColoresLinea, low= 110, high= 190)), color=transform('Cadencia[ppm]', LinearColorMapper(palette=PaletaColores, low= 110, high= 190)), fill_alpha=1)
    PLT_Linea = PLT.line(EjeX, 'Cadencia[ppm]', source= DatosBokeh, color= 'white', line_width=0, line_alpha=0)
    PLT.add_tools(HoverTool(tooltips=[('', '@{Cadencia[ppm]}{int} ppm')], renderers= [PLT_Linea], mode= 'vline'))
    ...
    for Metrica in MetricasAuxiliares:
        if DiccionarioVariables[Metrica]['Tipo'] == 'circle':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.circle(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['CADENCIA']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], size=SizeCircle, line_color=DiccionarioVariables[Metrica]['Color'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        elif DiccionarioVariables[Metrica]['Tipo'] == 'step':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.step(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['CADENCIA']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        else:
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.line(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['CADENCIA']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
    
    # Atributos
    PLT.title.text = 'CADENCIA'
    PLT.sizing_mode = 'fixed'
    PLT.yaxis.axis_label = 'Cadencia [Pasos por minuto]'
    PLT.yaxis.formatter = NumeralTickFormatter(format='0')
    PLT.grid.visible = False
    PLT.yaxis.minor_tick_line_color = None
    PLT.yaxis.major_label_overrides = FormateoEjes(DatosBokeh.data['Cadencia[ppm]'], 10, 1)
