...
from source.common.PaletasColores import Mint
...
def Altitud(DatosBokeh, EjeX, MetricasAuxiliares):
    """
        GRAFICO AREA | ALTITUD
    """
    ...
    PLT = figure(width=1000, height=400, x_range=(DatosBokeh.data[EjeX].min(), DatosBokeh.data[EjeX].max()), y_range=(LimiteEjeY(DatosBokeh, 'ALTITUD', 'Inferior'), LimiteEjeY(DatosBokeh, 'ALTITUD', 'Superior')), tools='', toolbar_location=None)
    
	# Creacion del area bajo la linea de la metrica a partir del CDS
    Area = DatosBokeh.to_df().copy().reset_index()[[EjeX, 'Altitud[m]']].set_index(EjeX)
    Area.rename(columns= {'Altitud[m]': 'Area'}, inplace=True)
    AreaBottom = Area[::-1]
    AreaBottom['Area'] = 0
    PLT.patch(x= hstack((AreaBottom.index, Area.index)), y= hstack((AreaBottom['Area'], Area['Area'])), color= Mint[1], alpha=1, line_color=None)
	
    # Inclusion de datos    
    PLT_Linea = PLT.line(EjeX, 'Altitud[m]', source=DatosBokeh, color=Mint[2], line_width=2, line_cap='round')

    # Efectos visuales en asfalto o montaña
    if (((DatosBokeh.data['DesnivelPositivo[m]'].max()+DatosBokeh.data['DesnivelNegativo[m]'].max())/DatosBokeh.data['Distancia[m]'].max())*1000) > 40:
        PLT.add_tools(HoverTool(tooltips=[('Distancia', '@{Distancia[m]}{0.0} m'),('Altitud', '@{Altitud[m]}{int} m'),('Desnivel positivo', '@{DesnivelPositivo[m]}{int} m'),('Pendiente', '@{Pendiente[%]}{0.00}%')], renderers= [PLT_Linea], mode= 'vline'))
    else:
        PLT.add_tools(HoverTool(tooltips=[('Distancia', '@{Distancia[m]}{0.0} m'),('Altitud', '@{Altitud[m]}{int} m')], renderers= [PLT_Linea], mode= 'vline'))
    ...
    for Metrica in MetricasAuxiliares:
        if DiccionarioVariables[Metrica]['Tipo'] == 'circle':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.circle(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['ALTITUD']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], size=SizeCircle, line_color=DiccionarioVariables[Metrica]['Color'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        elif DiccionarioVariables[Metrica]['Tipo'] == 'step':
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.step(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['ALTITUD']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
        else:
            ListadoMetricasAuxiliares['l' + str(MetricasAuxiliares.index(Metrica))] = PLT.line(EjeX, DiccionarioVariables[Metrica]['Variable'].split('[',1)[0] + DiccionarioVariables['ALTITUD']['Sufijo'], source=DiccionarioVariables[Metrica]['CDS'], color=DiccionarioVariables[Metrica]['Color'], **DiccionarioVariables[Metrica]['Propiedades'])
    
    # Atributos
    PLT.title.text = 'ALTITUD'
    PLT.sizing_mode = 'fixed'
    PLT.yaxis.axis_label = 'Altitud [m]'
    PLT.yaxis.formatter = NumeralTickFormatter(format='0')
    PLT.grid.visible = False
    PLT.yaxis.minor_tick_line_color = None
    PLT.yaxis.major_label_overrides = FormateoEjes(DatosBokeh.data['Altitud[m]'], 100, 1)
